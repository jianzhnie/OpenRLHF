...
work_dir="./work_dir/drgrpo_math_lvl3to5/hand-selected2k-${date}"
# 如果work_dir不存，则创建该目录
if [ ! -d "${work_dir}" ]; then
    mkdir -p ${work_dir}
fi
...
openrlhf.cli.train_ppo \
   ...
   --cosine_max_len 2048 \
   --prompt_data /root/llmtuner/hfhub/datasets/math_lvl3to5_selected2k.json \
   ...
   --wandb_project openrlhf_qwen2-5_7b_math_lvl3to5 \
   --wandb_group drgrpo \
   --wandb_run_name Qwen2.5-Math-7B-drgrpo-hand-selected2k
EOF
